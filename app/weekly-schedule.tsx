...
export default function WeeklySchedule() {
  ...
  const generateSchedule = async () => {
    ...
    const { data: groupRow } = await supabase.from("carpool_groups").select("consolidate_late_pickups").eq("id", groupId).single();
  ...
  const generateBasicSchedule = async () => {
    ...
    const { data: groupRow } = await supabase.from("carpool_groups").select("consolidate_late_pickups").eq("id", groupId).single();
    const newSlots = computeBasicSchedule(DAYS, availability || [], exceptions || [], members, !!groupRow?.consolidate_late_pickups);
    const today = new Date();
    const dayOfWeek = today.getDay();
    const monday = new Date(today);
    monday.setDate(today.getDate() - (dayOfWeek === 0 ? 6 : dayOfWeek - 1));
